...
/**
 * Builds an AIProvider on top of a given transport engine. All prompt
 * construction and schema validation is identical across backends — only
 * the engine (Anthropic vs Gemini) differs. See engine.ts.
 */
export function createAIProvider(engine: AIEngine): AIProvider {
  return {
    async generateStructuredRequirement(rawRequirement, overrides: StructuredInputOverrides, questionsAsked = 1) {
      ...
      return engine.callStructured(REQUIREMENT_SYSTEM_PROMPT, userPrompt, requirementJsonSchema, (raw) =>
    ...
    async generateJD(requirement) {
      ...
      return engine.callStructured(JD_SYSTEM_PROMPT, userPrompt, jdJsonSchema, (raw) => JDGenerationSchema.parse(raw)) as Promise<JDGeneration>;
    },

    async improveJD(currentJD, instruction) {
      const userPrompt = `Current job description:\n${JSON.stringify(currentJD, null, 2)}\n\nRecruiter instruction:\n"""\n${instruction}\n"""`;
      return engine.callStructured(IMPROVE_SYSTEM_PROMPT, userPrompt, jdJsonSchema, (raw) => JDGenerationSchema.parse(raw)) as Promise<JDGeneration>;
    },

    async evaluateCandidate(input) {
      const userPrompt = buildScreeningUserPrompt(input);
      return engine.callStructured(SCREENING_SYSTEM_PROMPT, userPrompt, candidateEvaluationJsonSchema, (raw) =>
    ...
    async generateInterviewPlan(input) {
      ...
      return engine.callStructured(INTERVIEW_PLAN_SYSTEM_PROMPT, userPrompt, interviewPlanJsonSchema, (raw) =>
    ...
    async generateQuestion(input) {
      ...
      return engine.callStructured(QUESTION_SYSTEM_PROMPT, userPrompt, interviewQuestionJsonSchema, (raw) =>
    ...
    async evaluateAnswer(input) {
      ...
      return engine.callStructured(ANSWER_EVALUATION_SYSTEM_PROMPT, userPrompt, answerEvaluationJsonSchema, (raw) =>
    ...
    async generateFollowUp(input) {
      ...
      return engine.callStructured(FOLLOWUP_SYSTEM_PROMPT, userPrompt, followUpDecisionJsonSchema, (raw) =>
    ...
    async evaluateInterview(input) {
      ...
      return engine.callStructured(INTERVIEW_EVALUATION_SYSTEM_PROMPT, userPrompt, interviewEvaluationJsonSchema, (raw) =>
    ...
    async generateAssessment(input) {
      ...
      return engine.callStructured(ASSESSMENT_GENERATION_SYSTEM_PROMPT, userPrompt, assessmentGenerationJsonSchema, (raw) =>
    ...
    async improveAssessment(input) {
      ...
      return engine.callStructured(IMPROVE_ASSESSMENT_SYSTEM_PROMPT, userPrompt, assessmentGenerationJsonSchema, (raw) =>
        AssessmentGenerationSchema.parse(raw)
      ) as Promise<AssessmentGeneration>;
    },
    ...
    async evaluateAssessmentAnswer(input) {
      ...
      return engine.callStructured(ASSESSMENT_QUESTION_EVALUATION_SYSTEM_PROMPT, userPrompt, assessmentQuestionEvaluationJsonSchema, (raw) =>
    ...
    async reviewOpenEndedSubmission(input) {
      ...
      return engine.callStructured(OPEN_ENDED_REVIEW_SYSTEM_PROMPT, userPrompt, openEndedReviewJsonSchema, (raw) =>
    ...
    async evaluateWorkdayTask(input) {
      ...
      return engine.callStructured(WORKDAY_TASK_EVALUATION_SYSTEM_PROMPT, userPrompt, workdayTaskEvaluationJsonSchema, (raw) =>
    ...
    async extractCandidateFromResume(resumeText) {
      ...
      return engine.callStructured(RESUME_CANDIDATE_EXTRACTION_SYSTEM_PROMPT, userPrompt, resumeCandidateExtractionJsonSchema, (raw) =>
    ...
    async explainCandidate(input) {
      ...
      return engine.callText(EXPLAIN_CANDIDATE_SYSTEM_PROMPT, userPrompt);
    },
  };
}
